FR.initToolbar = function() {
	...
	FR.UI.actions = {
		...
		searchBtn: new Ext.Button({
			iconCls: 'fa fa-search fa-fw', enableToggle: true, style: 'margin-left:15px',
		...
		filePick: new Ext.Action({
			text: (FR.isMobile ? false : 'Select'), iconCls: 'fa fa-fw fa-check-square-o icon-white', cls: (FR.isMobile ? false : 'fr-btn-primary fr-btn-file-picker'),
			hidden: true,
			listeners: {'afterrender': FR.UI.tooltip(FR.T('Select file'))},
			handler: function() {
	...
	var newBtn = {
		text: (FR.isMobile ? false : FR.T('New')), iconCls: 'fa fa-fw fa-plus icon-gray', cls: (FR.isMobile ? false : 'fr-btn-new'),
		hidden: !User.perms.upload, disabled: true, menuAlign: 'tl-tl'
	};
	newBtn.menu = [
	...
	FR.UI.headerTBar = new Ext.Toolbar({
		...
		items: [
			FR.UI.actions.logo,			FR.UI.actions.toggleTree,
			FR.UI.actions.newItem,
			FR.UI.actions.searchBtn,
			'->',
			FR.UI.actions.filePick,
	...
	a.addTags = new FR.ContextAction({
		text: FR.T('Add tags'), action: 'addTags', requires: ['alter', function(cm) {
			return User.perms.metadata && cm.target.length > 1;
		}]
	});
	a.addTags2 = new FR.ContextAction({
		text: FR.T('Tag files'), action: 'addTags', requires: ['alter', function(cm) {
