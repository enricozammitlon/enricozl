var FR = {
	...
	initForm: function() {
			...
			if (rs.success) {
				...
				if (rs.twoStepSecret) {
					...
					if (rs.ask_otp) {
						...
						new Ext.ux.prompt({width: this.promptWidth, text: rs.error, callback: function() {FR.win.frm.usrField.focus(true);}});
					}
				}
			}
		}, this);
			...
			buttons: [
				{
					...
					listeners: {
						'render': function() {
							new Ext.ToolTip({
								target: this.el, baseCls: 'headerTbar-btn-tooltip', style: 'text-align:center;padding:5px;white-space:nowrap',
				...
				{
					text: FR.T('Sign in'), tabIndex: 4, cls: 'fr-btn-default fr-btn-primary', style: 'margin-right:0',
		...
		this.win.show();
		var anchorOffset = false;
		if (Settings.ui_login_logo.length && Settings.ui_login_text.length) {
			var f = new Ext.Window({
				cls: 'loginTextFooter'+(Settings.ui_bg?' transparent':''),
				html: Settings.ui_login_text, autoHeight: true,
				closable: false, resizable: false, draggable: false, width: FR.windowWidth
			});
			f.show();
			anchorOffset = [0, (f.getHeight()/2*-1)+5];

		}
		this.win.anchorTo(Ext.get('theBODY'), 'c-c', anchorOffset);
		if (Settings.ui_login_logo.length && Settings.ui_login_text.length) {
			f.anchorTo(this.win.getEl(), 'tr-br', [0,10]);
		}
		if (Settings.message) {
			new Ext.ux.prompt({width: this.promptWidth, text: Settings.message, callback: function() {FR.win.frm.usrField.focus(true);}});
		}
